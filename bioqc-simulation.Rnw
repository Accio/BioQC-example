...
meanThrShow <- data.frame(Tissue=names(meanThr), sprintf("%1.2f%%", meanThr*100))
meanThrTbl <- xtable(meanThrShow,
                     caption="Average lower detection limites of tissues as contamination sources",
                     label="tbl:detectLimit")
print(meanThrTbl, floating=FALSE, tabular.environment="longtable")
@ 
\end{center}

Interestingly, brain samples are a special case: if they contaminate other tissues, it is more difficult to identify (but not other way around). We suspect this is due to the fact that many genes that are specifically expressed in brain are present at relative low levels; therefore although the prefrontal cortex signatures could correctly predict the brain sample in the quality control (Table~\ref{tbl:dogTissue}), they are less powerful detecting brain-causing contaminations. It remains to be tested whether this phenomenon is observed in other datasets and in species other than dog.

Otherwise, most \textit{in silico} contamination events are detectable in this dataset, with median detection limit around \Sexpr{median(comProp, na.rm=TRUE)}. This suggests that \Rpackage{BioQC} works well in physiological settings.
