...
We found that \Sexpr{nrow(diffTable)} probesets representing \Sexpr{length(diffGenes)} genes are associated with much stronger expression changes if the contaminated sample is not excluded (Table~\ref{tbl:diffTbl}). Not surprisingly almost all of these genes are highly expressed in normal human pancreas tissues, and \Sexpr{sum(diffGenesPancreas)} genes belong to the pancreas signature used by \Rpackage {BioQC}.

\begin{figure}
\begin{center}
<<doDeg, fig=TRUE, echo=FALSE, width=6, height=6>>=
<<deg>>
@ 
\end{center}
...
library(xtable)
diffXtable <- xtable(diffTable, digits=rep(2, ncol(diffTable)+1))
